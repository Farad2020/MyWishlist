...
namespace WishListTests
{
    public class AddMVCMiddlewareTests
    {
        ...
        public void UseEndpointsAdded()
        {
            ...
            Assert.True(true, "`Startup.cs`'s `Configure` method did not contain a call to `UseEnpoints` on `app` after `UseRouting` with an argument of `endpoints => { endpoints.MapDefaultControllerRoute(); }`");
        }
    }
}
